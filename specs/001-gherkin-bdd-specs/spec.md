...
- **FR-006**: The library MUST expose a module-level `solve()` function that accepts a `Problem` object and returns a `Result` object. The function MUST be importable directly from the top-level package: `from dwsolver import solve`.
- **FR-007**: The `Result` object MUST carry a `status` field with values from a defined set: `"optimal"`, `"infeasible"`, `"unbounded"`, `"iteration_limit"`. When status is `"infeasible"` or `"unbounded"`, `variable_values` MUST be empty and no objective value is reported. When status is `"iteration_limit"`, the best feasible solution found before the limit was reached MUST be returned: `variable_values` is populated and an objective value (for that incumbent) is present (see FR-011 for the required diagnostic message).
- **FR-011**: A `Result` with status `"iteration_limit"` MUST include a diagnostic message describing the limit reached (e.g., "maximum iteration count N reached") so the caller can decide whether to re-run with adjusted parameters. This is the sole authoritative statement on the diagnostic message requirement for `iteration_limit`; FR-007 defers to this requirement.
- **FR-012**: Both the CLI and the library MUST accept a caller-specified maximum worker count that controls how many subproblem blocks are solved concurrently. The CLI MUST expose this as a `--workers` flag (default: `os.cpu_count() * 2`). The library `solve()` function MUST accept it as a `workers` keyword argument. The solver MUST NOT hard-code a concurrency level. Note: `workers=None` resolves to `os.cpu_count() * 2` at runtime — this reflects that HiGHS releases the GIL during solving, allowing CPU-bound parallelism beyond `cpu_count()`. The effective thread-pool size is further capped at `len(blocks)` because spawning more threads than blocks provides no benefit; the actual pool size used is `min(resolved_workers, len(blocks))`.
...
- **FR-014**: The solver MUST terminate when the relative optimality gap falls at or below a configurable tolerance. The default tolerance MUST be `1e-6`. The CLI MUST expose this as a `--tolerance` flag and the `solve()` function MUST accept it as a `tolerance=` keyword argument. The tolerance value MUST be a named constant in the codebase, documented with its mathematical definition.
- **FR-008**: The library MUST raise a `DWSolverInputError` when the provided `Problem` object is structurally invalid; this exception type MUST be importable from the top-level package. Structural invalidity includes (but is not limited to): missing required fields, dimension mismatches, out-of-range indices, and duplicate `variable_names` across blocks (since `Result.variable_values` is a flat `dict[str, float]` keyed by variable name — duplicates would silently collide).
- **FR-009**: The library API MUST be stateless across calls — repeated calls to `solve()` with different problems MUST produce independent results.
...
- **Block**: One independent subproblem in the decomposition. Has its own variables, constraints, and objective. The master problem links blocks via linking constraints. Variable names declared in a block MUST be globally unique across all blocks in the `Problem`; duplicate names across blocks are a structural error.
